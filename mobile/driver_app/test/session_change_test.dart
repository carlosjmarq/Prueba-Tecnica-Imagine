...
Order _orderFor(String token) {
  final tag = token == 'driver-a' ? 'aaaaaaaa' : 'bbbbbbbb';
  return Order(
...
void main() {
  testWidgets('cambio de sesion no muestra pedidos del driver anterior',
      (tester) async {
    ...
    container.read(authSessionProvider.notifier).state =
        AuthSession(accessToken: 'driver-a', refreshToken: 'ra');

    await tester.pumpWidget(
      UncontrolledProviderScope(
        container: container,
        child: const MaterialApp(home: AvailableOrdersView()),
      ),
    );
    ...
    container.read(authSessionProvider.notifier).state =
        AuthSession(accessToken: 'driver-b', refreshToken: 'rb');
    await tester.pumpAndSettle();
